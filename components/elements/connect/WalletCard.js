...
`
const WalletTypeCard = styled(animated.div)`
    display: flex;
    flex-direction: column;
    align-content: center;
    justify-content: center;
...
const WalletCard = (props) => {
    const [exitAnimeTrigger, setExitAnimeTrigger] = useState(false)
    const [walletAnimeTrigger, setWalletAnimeTrigger] = useState(false)
    const exitStyle = useSpring({
        transform: exitAnimeTrigger
          ? `rotate(45deg)`
          : `rotate(0deg)`,
        config: {
          tension: 300,
      ...
      const walletAnimeStyle = useSpring({
        transform: walletAnimeTrigger
          ? `scale(1.1)`
          : `scale(1)`,
        config: {
          tension: 140,
    ...
    const buttonTrigger = () => {
        setExitAnimeTrigger(prev => !prev)
    }
    const walletTrigger = () => {
        setWalletAnimeTrigger(prev => !prev)
    }
    return (
        <>
        ...
        <FakeBackground>
            <ModalContainer>
                ...
                <ExitButton
                 ...
                 onMouseLeave={buttonTrigger} 
                 onClick={() => props.setShowDepositModal()}
                 >
                     <FaTimesCircle/>
                </ExitButton>
                    ...
                    <TitleContainer>
                    <TitleText>Select Wallet Provider</TitleText>
                    </TitleContainer>

                    <CardRow>
                        <WalletTypeCard
                         style={walletAnimeStyle}
                         onClick={() => props.handleConnect(injected)}
                         onMouseEnter={walletTrigger}
                         onMouseLeave={walletTrigger} 
                         >
